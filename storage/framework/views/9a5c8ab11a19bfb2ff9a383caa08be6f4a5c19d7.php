  ...
  $(document).ready(function() {
    $.ajax({
      ...
      success: function(data) {
        var id_kecam = `<?php echo e(Session::get('id_kecamatan')); ?>`
        const koordinator = data['anak'].filter((id_kecamatan) => id_kecamatan.id_kecamatan == id_kecam);
        // console.log(koordinator)
        // status anak
        const anak_yatim = koordinator.filter((status_anak) => status_anak.status_anak == 1);
        const anak_piatu = koordinator.filter((status_anak) => status_anak.status_anak == 2);
        const yatim_piatu = koordinator.filter((status_anak) => status_anak.status_anak == 3);

        const jumlah_yatim = anak_yatim.length / koordinator.length * 100;
        const jumlah_piatu = anak_piatu.length / koordinator.length * 100;
        const jumlah_yatim_piatu = yatim_piatu.length / koordinator.length * 100;
        // jenis kelamin
        const laki_laki = koordinator.filter((jenis_kelamin) => jenis_kelamin.jenis_kelamin == 1);
        const perempuan = koordinator.filter((jenis_kelamin) => jenis_kelamin.jenis_kelamin == 0);
        const jumlah_lk = laki_laki.length / koordinator.length * 100;
        const jumlah_pr = perempuan.length / koordinator.length * 100;
        ...
        for (let i = 0; i < data['desa'].length; i++) {
          ...
          if (data['desa'][i]['id_kecamatan'] == id_kecam) {
            const desa = data['anak'].filter((desa) => desa.id_desa == data['desa'][i]['id_desa']);
            let kec = {
...
<?php echo $__env->make('layouts.koordinator.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\ASUS\Music\skripsi\pmks_pengembangan_2-master\resources\views/koordinator/dashboard.blade.php ENDPATH**/ ?>